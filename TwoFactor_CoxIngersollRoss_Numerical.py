...
## ytm_exact=9.7885708784414313e-02 ## not used
...
# Derived quantities: specific to CIR for now, simplified from gen case */
for i in range(N):
    K[ i,i]=kap[i]
    Sig[ i,i]=sig[i]
    Kt[i,i]=kap[i]+lam[i]
...
for counter in range(-1,ntol+1):
    ...
    sol=solve_ivp(fun=ntrlfunc, t_span=(t,ti), rtol=rtol, atol=atol,y0=yntrl, 
                  method=method)
    ...
    if counter==-1: #reference solution
        ...
        print('tol{:.4e}  t{:.4e}  ytm{:.16e}  err{:.4e}'.format(tol,t, 
              ytmntrl,np.abs(ytmntrl-ytm_exact)))

#### Setting up The Yield Curve: ###
yields1=np.zeros([0]) #Creating NumPy array of zeros
yields1=(Y[0]+Y[1]) #NumPy array of bond yields
